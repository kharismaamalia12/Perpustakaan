...
if(isset($_POST['btnPinjam'])){
	...
	$sql = "INSERT INTO peminjaman(id_anggota, tgl_pinjam, tgl_jatuh_tempo, id_petugas) 
			VALUES('$id_anggota', '$tgl_pinjam', '$tgl_jatuh_tempo', 1)";
	
	$stok = ambilStok($kon, $id_buku);

	if(cekPinjam($kon, $id_anggota) && $stok > 0){
		$res 	= mysqli_query($kon, $sql);
		$query	= mysqli_query($kon,"SELECT id_pinjam FROM peminjaman WHERE tgl_pinjam='$tgl_pinjam' AND id_anggota='$id_anggota' AND 
				  tgl_jatuh_tempo='$tgl_jatuh_tempo' AND id_petugas=1 ");
